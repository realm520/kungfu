...
class MarketDataXT(pywingchun.MarketData):
    def __init__(self, low_latency, locator, config_json):
        pywingchun.MarketData.__init__(self, low_latency, locator, "restbn")
        self.config = MakerConfig(base=200.0, bound=1000, samples=1000,variation=4, randseed=6)
        self.api = Client("", "")
        self.orderbooks = {}
        self.logger = create_logger("restbn_md", "info", pyyjj.location( pyyjj.mode.LIVE, pyyjj.category.MD, "restbn", "restbn", locator))
        self.precision = 10 ** 18

    def on_start(self):
        self.add_time_interval(1000 * 1000 * 1000, lambda e: self.update_orderbooks())
